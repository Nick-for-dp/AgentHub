...
### Requirement: 工作台提供可筛选且可逻辑删除的最近工作记录

工作台 SHALL 展示当前登录用户创建且尚未逻辑删除的合同审查工作记录，并支持按合同文件名、任务状态和合同类型筛选。记录列表 MUST 分页且按创建时间倒序排列；用户点击记录后 SHALL 可重新加载对应合同解析文本与审查结果。删除操作 MUST 仅设置逻辑删除标记，不得物理删除合同解析任务、对象存储文件、审查结果或调用审计记录；非终态任务 MUST NOT 被删除。

#### Scenario: 当前用户筛选最近工作记录

- **WHEN** internal 用户输入合同文件名关键词，或选择任务状态、合同类型筛选条件
- **THEN** 工作台仅返回当前用户拥有、未逻辑删除且符合全部筛选条件的记录，并显示稳定分页信息

#### Scenario: 重新打开历史审查结果

- **WHEN** 用户点击一条可访问的最近工作记录
- **THEN** 工作台通过 AgentHub API 加载合同审查任务及其文件解析任务，并恢复展示任务状态、合同原文、审查摘要和条款结果

#### Scenario: 逻辑删除已结束的工作记录

- **WHEN** 用户确认删除一条 SUCCEEDED、FAILED 或 CANCELLED 的本人工作记录
- **THEN** 系统记录删除时间和操作用户，该记录不再出现在列表或普通详情查询中，同时合同文件、解析结果和调用审计数据仍被保留

#### Scenario: 运行中的工作记录不能删除

- **WHEN** 用户尝试删除 PENDING 或 RUNNING 的合同审查任务
- **THEN** 系统拒绝删除并保持任务可查询，前端说明任务结束后才能删除
