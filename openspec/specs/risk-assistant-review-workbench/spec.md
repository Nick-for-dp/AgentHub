# risk-assistant-review-workbench

为 internal 用户提供风控任务提交、找回、审计信息查看、证据访问和人工复核工作台。

## Purpose

让内部用户通过稳定 Web 界面完成多文件风控任务的提交、状态恢复、完整审计上下文查看和同任务人工复核，同时保持前端与 LangGraph、OCR/VLM 和对象存储实现解耦。
