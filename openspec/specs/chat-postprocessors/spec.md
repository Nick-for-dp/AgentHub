...
chat 流结束后的可插拔后处理器链：归一化输出消费、线索收集等副作用按 Agent 配置启用，且不得泄漏 runtime provider。

## Purpose

将线索收集等 chat 完成后副作用从 endpoint 硬编码中抽离，使非问答 Agent 默认不跑问答专属逻辑，并允许按 `config_snapshot.postprocessors` 显式开关。
