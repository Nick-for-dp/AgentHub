...
- 新增 external/internal 两套部署模板，包括独立环境变量示例、systemd unit、Nginx upstream/server、静态产物目录、日志标识、迁移/启动/回滚步骤和健康检查。
...
- 部署：新增 `deploy/profiles/external/`、`deploy/profiles/internal/`、双端口 Nginx 配置、两套 systemd unit、环境变量模板、安装/迁移/seed/回滚/验收文档；现有单实例模板保留兼容说明或收敛为共享基础模板。
