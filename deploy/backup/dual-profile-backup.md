...
两个 `.my.cnf` 应使用不同的只读备份账号，分别只授予目标 schema 的 `SELECT, LOCK TABLES, SHOW VIEW, EVENT, TRIGGER`。脚本使用 `--no-tablespaces`，不需要授予全局 `PROCESS` 权限。恢复演练应写入各自的隔离恢复库，核对 Alembic revision、关键资源数量和 profile 专属 Agent 后再切换；不得把 external 备份恢复到 internal schema，反之亦然。
